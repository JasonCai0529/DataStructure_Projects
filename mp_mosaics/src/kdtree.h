...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* buildNode(RandIter start, RandIter end, int n, int d, int k_dim);

    KDTreeNode* copyTree(KDTreeNode* rhs);
    KDTreeNode* find_node(KDTreeNode* cur_root, const Point<Dim>& query, int d) const;
};
